          ...
          <v-card class="mx-auto" max-width="800" color="accent">
            <v-card-text class="title" >
                  <v-row justify="center" no-gutters>
                    ...
                    <v-col xs="2">
                       <select v-model="selected" class="font-weight-medium" @change=" roundNumber(selected)">
                        <option disabled value=""> Currency </option>
                        <option v-for="(value, key) in rates" :key="key" :value="value">{{ key }}</option>                      
                      </select>
                    </v-col>
                   <v-col xs="2">
                      <span dark>Rate: {{ round }}</span>                     
                   </v-col>                  
                  </v-row>
            </v-card-text>
          </v-card>
...
export default {
  ...
  methods: {
    ...mapMutations(['mostrarDialog', 'ocultarDialog', 'mostrarAlert']),
      ...
      try {
        ...
        this.mostrarAlert({titulo:'No fue posible acceder a la API', color:'red lighten-1'})
      }finally{
        this.ocultarDialog()
      }
    },
